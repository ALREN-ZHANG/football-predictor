...
# ========== 历史记录管理 ==========
RESULTS_FILE = "match_results1.csv"
...
def save_result(date, home_team, away_team, home_score, away_score, handicap, actual_handicap, predict_result,
    ...
    try:
        ...
        st.error("❌ 无法保存比赛结果，请关闭可能占用 'match_results1.csv' 文件的程序（如Excel），然后重试。")
        return None
    except Exception as e:
...
with tab3:
    ...
    if dfh.empty:
        ...
        if df_display.empty:
            ...
            st.download_button("导出当前筛选结果", csv_hist, "match_results1.csv", "text/csv")
